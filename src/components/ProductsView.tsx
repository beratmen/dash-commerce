...
import { Container, Card, CardContent, CardMedia, Typography, CircularProgress, Box, Pagination, Rating, Button,} from '@mui/material';  // Material-UI UI bileşenleri (kart, yazı, yükleme göstergesi, buton vb.)
import { ShoppingCart } from '@mui/icons-material';  // Sepet ikonu (Add to Cart butonunda)
import Link from 'next/link';  // Next.js sayfalar arası bağlantı (ürün detay sayfasına gitme)
import { useAppDispatch, useAppSelector } from '@/store/hooks';  // Redux dispatch ve selector hook'ları
import { fetchProductsAsync, setProducts } from '@/store/slices/productSlice';  // Ürün state aksiyon'ları
import { setPage } from '@/store/slices/uiSlice';  // Sayfa numarası değişim aksiyon'u
import { addToCart } from '@/store/slices/cartSlice';  // Sepete ekleme aksiyon'u
import { ProductsViewProps } from '@/types';  // Prop'ların TypeScript tip tanımları
...
export default function ProductsView({ initialData }: ProductsViewProps) {
    ...
    const handlePageChange = (_: React.ChangeEvent<unknown>, page: number) => { //{_}bu parametreyi alıyorum ama kullanmayacağım" anlamına gelir
                    ...
                    <Box key={String(product.id)} sx={{ height: '100%' }}>
                        ...
                        <Link href={`/products/${product.id}`} style={{ textDecoration: 'none' }}>
                            ...
                            <Card sx={{
                                ...
                                <CardContent sx={{ flexGrow: 1 }}>  {/* flexGrow: Boş alan var ise bu bölüm genişlet her zaman aynı hizada, en altta görünür. */}
                                    ...
                                    {/* KATEGORİ */}
                                    <Typography variant="body2" color="text.secondary" gutterBottom>  {/* ufak bir alt boşluk ekle */}
                                        {product.category}
                                    </Typography>
