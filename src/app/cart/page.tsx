'use client'; // Bu sayfa kullanıcı etkileşimi (buton tıklamaları vb.) içerdiği için Client Component olmalıdır.
...
import { useAppDispatch, useAppSelector } from '@/store/hooks'; // Redux state ve dispatch için özel hook'lar
...
/*
 * CartPage Component
 * Kullanıcının sepetindeki ürünleri listelediği, miktarları güncellediği 
 * ve ödeme adımına geçtiği ana sepet sayfası.
 */
export default function CartPage() {
    const theme = useTheme();
    // Masaüstü görünümü için breakpoint kontrolü (Sepet özetini sticky yapmak için kullanılır)
    const isDesktop = useMediaQuery(theme.breakpoints.up('md'));
    const dispatch = useAppDispatch();

    // Redux store'dan sepet verilerini çekiyoruz
    const items = useAppSelector(selectCartItems); // Sepetteki ürün listesi
    const totalPrice = useAppSelector(selectCartTotalPrice); // Toplam fiyat
    const totalQuantity = useAppSelector(selectCartTotalQuantity); // Toplam ürün adedi

    /**
     * Ürün miktarı güncelleme fonksiyonu
     * @param productId Güncellenecek ürünün ID'si
     * @param delta Artış/Azalış miktarı (1 veya -1)
     */
    ...
    // SEPET BOŞSA: Kullanıcıyı bilgilendiren ve alışverişe yönlendiren ekranı göster
    if (items.length === 0) {
        return (
            <Box sx={{ minHeight: '70vh', display: 'flex', alignItems: 'center', justifyContent: 'center', py: 8, px: 2 }}>
                <Container maxWidth="sm">
                    ...
                    <Paper elevation={0} sx={{ p: 6, textAlign: 'center', borderRadius: 4, bgcolor: '#f8fafc', border: '1px solid', borderColor: 'divider' }}>
                        ...
                        <Typography color="text.secondary" sx={{ mb: 3 }}>Explore our amazing products and fill your cart.</Typography>
                        <Button component={Link} href="/products" variant="contained" size="large">Start Shopping</Button>
                    </Paper>
                </Container>
            </Box>
        );
    ...
    // SEPET DOLUYSA: Ürün listesini ve sepet özetini göster
    return (
        <Container maxWidth="lg" sx={{ py: { xs: 3, md: 6 }, pb: 8 }}>
            ...
            {/* Başlık ve Toplam Ürün Sayısı */}
            <Box display="flex" flexWrap="wrap" alignItems="baseline" gap={2} sx={{ mb: 4 }}>
                <Typography variant="h4" fontWeight="800" sx={{ color: '#0f172a' }}>My Cart</Typography>
            ...
            {/* İki Sütunlu Layout: Sol taraf ürün listesi, sağ taraf özet paneli */}
            <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: '1fr 380px' }, gap: 4, alignItems: 'start' }}>
                        ...
                        <Paper key={item.productId} elevation={0} sx={{ p: 2, borderRadius: 3, border: '1px solid', borderColor: 'rgba(226,232,240,0.9)', '&:hover': { borderColor: 'rgba(13,148,136,0.25)' } }}>
                            <Box sx={{ display: 'flex', flexDirection: { xs: 'column', sm: 'row' }, gap: 2, alignItems: { xs: 'stretch', sm: 'center' } }}>
                                ...
                                <Box sx={{ flex: 1, minWidth: 0 }}>
                                    ...
                                    <Typography variant="body2" color="text.secondary" sx={{ mt: 0.5 }}>Unit Price: ${item.price}</Typography>
                                    
                                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, mt: 2 }}>
                                        {/* Arttırma/Azaltma Buton Grubu */}
                ...
                {/* SAĞ TARAF: SEPET ÖZETİ (Siparişi Tamamla) */}
                <Paper elevation={0} sx={{ borderRadius: 3, overflow: 'hidden', border: '1px solid rgba(13,148,136,0.2)', ...(isDesktop && { position: 'sticky', top: 24 }) }}>
                    ...
                    <Box sx={{ px: 3, py: 2.5, background: 'linear-gradient(135deg, rgba(13,148,136,0.1) 0%, rgba(15,118,110,0.05) 100%)', borderBottom: '1px solid rgba(13,148,136,0.15)' }}>
                        <Typography variant="overline" sx={{ color: 'primary.main', fontWeight: 700, letterSpacing: 1.2 }}>Order Summary</Typography>
                        <Typography variant="h3" component="p" sx={{ fontWeight: 800, color: 'primary.main', mt: 0.5 }}>${totalPrice.toFixed(2)}</Typography>
                        <Typography variant="body2" color="text.secondary" sx={{ mt: 0.5 }}>There are {totalQuantity} items in your cart</Typography>
                    </Box>
                    
                    {/* Ödeme Adımına Geçiş Butonu */}
                    <Box sx={{ p: 3 }}>
